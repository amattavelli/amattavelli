...
…
…
Nella Tabella 1 abbiamo le vendite per Cliente nella Tabella 2 L’anagrafica Clienti ci serve riportare la nazione nella Tabella 2. Niente di più facile basta collegare i campi Id con la formula
CERCA.VERT(…)
Se aggiungiamo la colonna partita Iva tra la colonna … e la colonna …  la formula però restituisce un risultato indesiderato.
…
…
Il risultato era prevedibile: la funzione … restituisce sempre la terza colonna e ora nella terza colonna c’è la partita IVA mentre la nazione occupa ora la quarta colonna. Occorre una funzione che individui la posizione di un determinato elemento in un elenco, questa funzione è appunto CONFRONTA()
