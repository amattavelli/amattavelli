...
Il flusso di cassa rappresenta il cuore pulsante di ogni attività imprenditoriale. Una gestione inadeguata può portare a una crisi di liquidità, compromettendo la capacità di un'azienda di operare e rispettare i propri obblighi finanziari. Tradizionalmente, il monitoraggio del cash flow è basato su fogli di calcolo e processi manuali, che sebbene affidabili, spesso risultano di lento aggiornamento e soggetti a errori di compilazione e in caso di una conoscenza non approfondita degli strumenti, anche di progettazione.
Grazie ai recenti progressi tecnologici, l’AI offre strumenti avanzati in grado di automatizzare e migliorare l’analisi dei dati finanziari. Ciò consente di ottenere una visione chiara e tempestiva delle entrate e uscite di cassa, identificando potenziali problemi prima che diventino critici.
Quando si utilizza l'intelligenza artificiale, è essenziale non trattare argomenti che non conosciamo bene. Il rischio è di incorrere in errori comuni dei chatbot, soprattutto se non si formulano domande chiare a causa della nostra scarsa conoscenza.
Nel caso dei flussi di cassa, è consigliabile, qualora non si avesse una conoscenza approfondita dell'argomento, rivedere alcuni concetti consultando testi specifici o facendosi assistere dalla AI, che in questo contesto può essere considerata come un vero e proprio personal trainer per un ripasso della materia.
Solo padroneggiando le componenti di un rendiconto finanziario potremo interrogare la AI in merito a simulazioni e what if analysis e verificarne l’efficacia delle risposte fornite.
...
In un primo test l’elenco B non conteneva alcuni elementi dell’elenco A: l’AI ha riconosciuto facilmente i dati non corrispondenti. È bastato però aggiungere degli alimenti in B non contenuto in A per cominciare a ricevere risposte non corrette e allucinate.
...
‍️ Riesci a creare un tool per eseguire le riconciliazioni bancarie? Il tool dovrà accogliere due elenchi e individuare gli elementi non corrispondenti (o ripetuti un numero di volte diverso) in maniera da ricostruire il saldo in maniera corretta aggiungendo la riconciliazione della differenza pari a  Somma importi solo in Lista 1 - Somma importi solo in Lista 2 + Somma differenze per frequenza= differenza riconciliata.
...
Chiarito l’approccio corretto, possiamo osare un po’ di più e poiché il miglior modo per comprendere quali risultati si possono ottenere dalla AI è chiederlo direttamente. Abbiamo quindi formulato il seguente prompt:
...
Si sarebbero potuti ottenere risultati migliori con Office (Excel e Power BI), ma a quale costo di tempo e competenze?
...
Poiché stiamo chiedendo un ragionamento, ad oggi lo strumento più performante è sicuramente O1, in attesa che venga rilasciato il già annunciato O3.
Abbiamo quindi domandato:
...
Normalmente l’AI tende a rettificare quando le si segnala un disaccordo, ma questa volta O1 non rettifica e non chiede scusa, non trattandosi di un errore ed essendo le argomentazioni del tutto fondate.